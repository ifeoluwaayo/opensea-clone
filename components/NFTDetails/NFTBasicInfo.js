...
const style = {
	...
	actionItemContainer: `flex items-center justify-center cursor-pointer p-3`,
	icon: `h-6 w-6 text-gray-500 dark:text-gray-400`,
	assetTitle: `text-4xl font-bold text-gray-700 pt-6 dark:text-gray-100`,
	subInfoContainer: `hidden lg:block`,
};

const NFTBasicInfo = ({ name }) => {
	const actionItems = [
		{ icon: <BiRefresh className={style.icon} />, title: "Refresh" },
		{ icon: <BiLinkExternal className={style.icon} />, title: "Link" },
		{ icon: <MdShare className={style.icon} />, title: "Share" },
		{ icon: <MdMoreVert className={style.icon} />, title: "More" },
	];
